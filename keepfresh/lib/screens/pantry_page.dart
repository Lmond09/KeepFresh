// Place this in lib/screens/pantry_page.dart
...
class PantryPage extends StatefulWidget {
  ...
  const PantryPage({super.key, required this.username});
...
class _PantryPageState extends State<PantryPage> {
  ...
  String? _selectedStorageType = 'Pantry';
  ...
  Future<void> _loadSortPreferences() async {
    ...
    setState(() {
      _sortOption = SortOption.values[prefs.getInt('${widget.username}_pantry_sortOption') ?? 1];
      _sortAscending = prefs.getBool('${widget.username}_pantry_sortAscending') ?? true;
    });
  }

  Future<void> _saveSortPreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('${widget.username}_pantry_sortOption', _sortOption.index);
    await prefs.setBool('${widget.username}_pantry_sortAscending', _sortAscending);
  }

  Future<void> _loadItems() async {
    final allItems = await StorageService.loadFridgeItems(widget.username);
    setState(() {
      _items = allItems.where((item) => item.storageType == 'Pantry').toList();
      _filterItems();
    });
  }
