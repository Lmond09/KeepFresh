...
class FridgePage extends StatefulWidget {
  ...
  const FridgePage({super.key, required this.username});
...
class _FridgePageState extends State<FridgePage> {
  ...
  String? _selectedStorageType = 'Fridge';
  ...
  Future<void> _loadSortPreferences() async {
    ...
    setState(() {
      _sortOption = SortOption.values[prefs.getInt('${widget.username}_fridge_sortOption') ?? 1];
      _sortAscending = prefs.getBool('${widget.username}_fridge_sortAscending') ?? true;
    });
  }

  Future<void> _saveSortPreferences() async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setInt('${widget.username}_fridge_sortOption', _sortOption.index);
    await prefs.setBool('${widget.username}_fridge_sortAscending', _sortAscending);
  }

  Future<void> _loadItems() async {
    final allItems = await StorageService.loadFridgeItems(widget.username);
    setState(() {
      _items = allItems.where((item) => item.storageType == 'Fridge').toList();
      _filterItems();
    });
  }
  ...
  void _deleteItem(int index) async {
    final allItems = await StorageService.loadFridgeItems(widget.username);
    final targetItem = _filteredItems[index];
            ...
            child: Row(
              ...
              children: [
                DropdownButton<SortOption>(
                  ...
                  items: const [
                    DropdownMenuItem(value: SortOption.name, child: Text('Sort by Name')),
                    DropdownMenuItem(value: SortOption.quantity, child: Text('Sort by Quantity')),
                  ],
                ),
                IconButton(
                  icon: Icon(_sortAscending ? Icons.arrow_upward : Icons.arrow_downward, color: Colors.teal),
  ...
  List<Widget> _buildGroup(String title, List<FridgeItem> items, Color color) {
    return [
      Padding(
        padding: const EdgeInsets.all(12),
